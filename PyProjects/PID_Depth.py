...
# PID setup
pid = PID(1.0, 0.1, 0.05, setpoint=254)  # Setpoint is measured in milimeters
...
def livestream_from_camera(camera_index=0):
    ...
    while True:
        ...
        frame_with_contours, width = find_and_draw_contours(masked_frame, mask)

        if width is not None:
            # Compute the control output using PID
            control = pid(width)
            # Output the width and control values to the terminal
            print(f'Width: {width}, Control: {control:.2f}')
