...
# PID setup
frame_height = 480  # Updated camera frame height
center_y = frame_height / 2
pid = PID(0.1, 0.01, 0.005, setpoint=center_y)  # Adjusted PID gains
pid.output_limits = (-100, 100)  # Adjust as needed for your application

# Queue for smoothing
smooth_window_size = 5
center_y_queue = deque(maxlen=smooth_window_size)
...
def find_and_draw_contours(frame, mask):
    ...
    return frame, center_y_box

def smooth_center_y(center_y_box):
    # Add the current center_y_box value to the queue
    center_y_queue.append(center_y_box)
    
    # Calculate the smoothed value
    smoothed_center_y = np.mean(center_y_queue)
    
    return smoothed_center_y
...
def livestream_from_camera(camera_index=0):
    ...
    while True:
        ...
        frame_with_contours, center_y_box = find_and_draw_contours(masked_frame, mask)

        if center_y_box is not None:
            # Smooth the center_y_box value
            smoothed_center_y = smooth_center_y(center_y_box)
            
            # Compute the control output using PID
            control = pid(smoothed_center_y)
            # Output the center y and control values to the terminal
            print(f'Smoothed Center Y: {smoothed_center_y:.2f}, Control: {control:.2f}, Error: {smoothed_center_y - center_y:.2f}')
